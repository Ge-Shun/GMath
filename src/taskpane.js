// GMath Word 加载项 —— 核心逻辑
// 职责很单一：用 MathLive 可视化/LaTeX 编辑公式，点「插入到 Word」生成 Word 原生公式。
// 链路：MathLive 编辑 → 输出 MathML → mml2omml 转 OMML → 包成 Flat OPC → insertOoxml 插入 Word。
// 插入的是 Word 原生公式，之后双击/点进去即可用 Word 自带「公式工具」编辑。

import { mml2omml } from "./mathml2omml.js"; // 自研转换器，无第三方依赖

const BUILD = "2026-06-09-h"; // 版本标记：用于确认面板加载的是不是最新代码
...
async function insertEquation() {
  ...
  try {
    ...
    setStatus("已插入 Word 原生公式（点进去可用 Word 公式工具编辑）。", "ok");
  } catch (e) {
    // Office 的错误对象带 code / debugInfo，比 message 详细得多
    const detail = { code: e.code, message: e.message, debugInfo: e.debugInfo };
